...
//Main routine to translate EVL Response
function parse_Response(strInput){
	arrResponse = strInput.split(',');
	//console.lo(arrResponse[1]);
	return evl_ResponseType[arrResponse[0]].name +'\n'+ 
			evl_Partition_Status_Code[arrResponse[1]].description + '\n' + 
			iconLED(hex2bin(arrResponse[2]).result) +'\n'+
			'User:' + arrResponse[3] + '\n' +
			BEEP_field[arrResponse[4]]+ '\n' + 
